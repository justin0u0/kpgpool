...
int sockops_prog(struct bpf_sock_ops *skops) {
	...
	case BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB: // SYN-ACK
		...
		if (skops->local_port == POOLER_PORT) {
			/*
			key = bpf_ntohl(skops->remote_port);
			ret = bpf_sock_map_update(skops, &sockmap, &key, BPF_ANY);
			bpf_printk("[BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB] sockmap update: key=[%u], ret=[%d]", key, ret);
			*/
	...
	case BPF_SOCK_OPS_ACTIVE_ESTABLISHED_CB: // SYN
		...
		if (bpf_ntohl(skops->remote_port) == BACKEND_PORT) {
			/*
			key = skops->local_port + MAX_PORTS;
			ret = bpf_sock_map_update(skops, &sockmap, &key, BPF_ANY);
			bpf_printk("[BPF_SOCK_OPS_ACTIVE_ESTABLISHED_CB] sockmap update: key=[%u], ret=[%d]", key, ret);
			*/
...
int sk_skb_stream_verdict_prog(struct __sk_buff *skb)
{
	...
	bpf_printk("[sk_skb_stream_verdict_prog] code=[%u], len=[%u]", *code, *len);
	...
	if (skb->local_port == POOLER_PORT) {
		key = bpf_ntohl(skb->remote_port);
	} else if (bpf_ntohl(skb->remote_port) == BACKEND_PORT) {
		key = skb->local_port + MAX_PORTS;
	} else {
		return SK_PASS;
	}

	int ret = bpf_sk_redirect_map(skb, &sockmap, key, BPF_F_INGRESS);
	bpf_printk("[sk_skb_stream_verdict_prog] sockmap redirect: key=[%u], ret=[%d]", key, ret);
	return ret;
	*/

	// client [->] pooler -> backend [->] pooler -> client
